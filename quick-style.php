...
if(isset($headerbg)){
	$output .= '.header{ background-color: '.esc_attr($themeum_options['header-bg']) .'; }';
}

# Header Margin Bottom
if(isset($themeum_options['header-margin-bottom'])){
	$output .= '.site-header{ margin-bottom: '. (int) esc_attr($themeum_options['header-margin-bottom']) .'px; }';
}
...
if ($themeum_options['header-fixed']){
	if(isset($headerbg)){
		$output .= '#masthead.sticky, .header{ background-color: rgba('.esc_attr(themeum_hex2rgb($headerbg)).',.95); }';
	}
	...
	if(isset($headerbg)){
		$output .= '.header{ background-color: rgba('.esc_attr(themeum_hex2rgb($headerbg)).',.95); }';
	}
}
...
if (isset($banner_image) && !empty($banner_image) == '') {
	$output	.= '.sub-title-inner h2{background-image: url('.$banner_image.');background-repeat: no-repeat;background-position: center;'; 
}
