...
add_shortcode( 'themeum_person', function($atts, $content = null) {
	...
	extract(shortcode_atts(array(
        ...
        if($desg_color != ''){
            $desg_color = 'color:'.$desg_color.';';
        }
        if($btn_padding != ''){
            $btn_padding = 'padding:'.$btn_padding.';';
        }
        if($btn_size != ''){
            $btn_size = 'font-size:'.$btn_size.';';
        }
		...
		$output  	 = '<div class="themeum-person '.esc_attr($class).'">';
			...
			$output  	.= '<div class="person-details">';
				if ($person_name) $output 	.= '<h3 class="person-title" style="'.$title_color.'">' . esc_attr($person_name) . '</h3>';
				if ($person_deg) $output 	.= '<h4 class="person-deg"  style="'.$desg_color.'">' . esc_attr($person_deg) . '</h4>';
			$output  	.= '</div>';//person-details	
			$output  	.= '<div class="social-icon">';
				$output  	.= '<ul class="list-unstyled list-inline" style="'.$social_color.'">';
			...
			if($btn_url){
			$output  	.= '<p><a href="' . esc_url($btn_url) . '" class="btn" style="'.$btn_color.$btn_bg.$btn_padding.$btn_size.'">' . esc_attr($btn_text) . '</a></p>';
			}
...
if (class_exists('WPBakeryVisualComposerAbstract')) {
	vc_map(array(
		...
		"params" => array(		
			...
			array(
				"type" => "colorpicker",
				"heading" => __("Person Name Color", "themeum"),
			...
			array(
				"type" => "colorpicker",
				"heading" => __("Social Color", "themeum"),
