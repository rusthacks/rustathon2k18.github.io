    ...
    if(count($posts)>1){
    	...
    	$output .= '<ul class="latest-review '.esc_attr( $class ).'">';
		    ...
		    $output .= '<li class="latest-review-single-item review-' . esc_attr( $column ) . '">';
				...
				if ( $show_date == 'yes') {
					$output .= '<span class="entry-date" style="'.$date_color.'">';
					$output .= get_the_date('d M Y');
					$output .= '</span>';	
				}
				$output .= '<h3 class="entry-title"><a style="'.$title_color.'" href="'.get_permalink().'">'. get_the_title() .'</a></h3>';
			$output .= '</li>';
		    endforeach;
		    wp_reset_postdata();   
	    $output .= '</ul>';
...
if (class_exists('WPBakeryVisualComposerAbstract')) {
	vc_map(array(
		"name" => esc_html__("Themeum Latest Post", 'eventum'),
		"base" => "themeum_review",
		...
		"params" => array(				
			...
			array(
				"type" => "colorpicker",
				"heading" => esc_html__("Date Color", 'eventum'),
